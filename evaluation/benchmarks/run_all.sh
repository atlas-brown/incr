...
#   --run-mode bash|incr|both|incr-observe|all   default: both
#       incr = try+strace; incr-observe = observe; all = bash + incr + incr-observe per script
#   --skip-setup
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --run-mode=*)    RUN_MODE="${1#--run-mode=}"; shift ;;
        --run-mode)      shift; RUN_MODE="$1"; shift ;;
        --results-dir=*) RESULTS_DIR="${1#--results-dir=}"; shift ;;
        --only=*)
            append_only_list "${1#--only=}"
            shift
            ;;
        --only)
            shift
            [[ $# -eq 0 ]] && { echo "[run] ERROR: --only needs a benchmark list" >&2; exit 1; }
            append_only_list "$1"
            shift
            ;;
        --skip-setup)    SKIP_SETUP=1; shift ;;
        --clear-cache)   CLEAR_CACHE=1; shift ;;
        --no-clear-cache) CLEAR_CACHE=0; shift ;;
        --clear-outputs) CLEAR_OUTPUTS=1; CLEAR_OUTPUTS_SET=1; shift ;;
        --no-clear-outputs) CLEAR_OUTPUTS=0; CLEAR_OUTPUTS_SET=1; shift ;;
        --help|-h)
            sed -n '2,19p' "$0"
            exit 0
            ;;
        easy|full)       MODE=$1; shift ;;
        min|small)       SIZE=$1; shift ;;
        bash|incr|both|incr-observe|all) RUN_MODE=$1; shift ;;
        *)
