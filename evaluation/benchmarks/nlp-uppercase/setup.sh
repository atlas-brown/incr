#!/bin/bash
# setup.sh: idempotently install dependencies and fetch data for the nlp-uppercase benchmark.
...
if [[ "$size" == "min" ]]; then
    if [[ ! -d "$INPUT_DIR/pg-min" ]]; then
        echo "[setup] nlp-uppercase: copying min inputs from repo..."
        cp -r "$BENCHMARK_DIR/min_inputs/pg-min" "$INPUT_DIR/pg-min"
    fi
else
    echo "[setup] nlp-uppercase: fetching $size inputs..."
    bash "$BENCHMARK_DIR/fetch.sh" "--$size"
fi

echo "[setup] nlp-uppercase: done."
