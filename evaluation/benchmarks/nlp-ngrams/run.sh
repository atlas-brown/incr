#!/bin/bash
# run.sh: run the nlp-ngrams benchmark.
...
echo "[run] nlp-ngrams: mode=$RUN_MODE size=$RUN_SIZE"
...
DEFAULT_SCRIPTS=(ngrams-1.sh ngrams-2.sh ngrams-3.sh)
finalize_benchmark_scripts "$SCRIPT_DIR" "${DEFAULT_SCRIPTS[@]}"
run_benchmark_scripts "${SCRIPTS[@]}"
echo "[run] nlp-ngrams: done. Results: $TIME_FILE"
