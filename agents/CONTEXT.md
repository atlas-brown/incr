...
**Harness:** `measure()` in `run_lib.sh` only prints **`[run] WARNING`** when a timed script exits with a code **other than 0 or 1** (POSIX tools such as `grep` use exit **1** for “no match,” which is normal). **`verify_outputs.sh`** diffs bash vs incr stdout and also fails if **htslib/samtools-style** missing-input / bad-index lines appear in `*.err` for benchmarks **other than `file-mod`** (ffmpeg writes banners to stderr there). Stdout agreement alone is not sufficient if both sides failed the same way.
...
## Known Environment Issue: incr stdin hang
`incr` reads all of stdin before proceeding (stdin hash = part of cache key).
In non-terminal environments (CI, cursor shell), stdin is a persistent pipe that never closes.
Fix: redirect stdin to `/dev/null` when calling incr.sh from `run_lib.sh`:
```bash
time "$TOP/incr.sh" "$SCRIPT_DIR/$script" "$cache_dir" < /dev/null >"$out_file" 2>"$err_file"
```
Benchmark scripts read from files via env vars, not stdin, so this is safe.
...
| bio          | `fetch.sh --min` wget one BAM: **HG00421** from the same **medium** (small-tier) URL as multi-sample small fetches (~233 MB); `input_min.txt` is `CHS HG00421` (matches `bio-1.sh`). No separate `min_inputs/` synthetic BAM. | `inputs/bio-min/HG00421.bam` |
