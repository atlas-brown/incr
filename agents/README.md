...
└── docs/
    ...
    ├── EVALUATION_BENCHMARK_SUITE.md  # Evaluation suite (run_all.sh, run modes)
    ├── BENCHMARK_RUN_CONTEXT.md      # Operations checklist, troubleshooting
...
bash agents/test_incr_observe.sh edge    # filter

# Run benchmark
bash agents/run_bench.sh

# Generate plot (requires matplotlib)
python3 agents/benchmarks/plot.py agents/benchmarks/results.txt

# Full evaluation suite — see docs/EVALUATION_BENCHMARK_SUITE.md
bash evaluation/benchmarks/run_all.sh --mode easy --size min --run-mode all
# or: bash evaluation/run.sh --mode easy --size min --run-mode all
```
