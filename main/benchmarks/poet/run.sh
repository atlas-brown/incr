...
echo "[run] poet: mode=$RUN_MODE size=$RUN_SIZE"
...
DEFAULT_SCRIPTS=(poet-1.sh poet-2.sh poet-3.sh poet-4.sh)
finalize_benchmark_scripts "$SCRIPT_DIR" "${DEFAULT_SCRIPTS[@]}"
run_benchmark_scripts "${SCRIPTS[@]}"
echo "[run] poet: done. Results: $TIME_FILE"
